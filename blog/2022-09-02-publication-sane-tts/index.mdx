...
쉽게 말하자면 화자 A의 목소리로도 자연스럽게 영어 음성을 합성할 수 있어야 합니다.
이렇게 훈련 데이터셋에서 특정 화자가 사용한 언어와 다른 언어로 음성을 합성하는 것을 cross-lingual speech synthesis라 부르며, 위와 같은 이유로 다국어 TTS 모델을 구현하기 위해 cross-lingual speech synthesis을 자연스럽고 안정적으로 만드는 일은 매우 중요합니다.
...
Duration predictor에서는 기본 VITS 세팅에서의 speaker embedding과 동일하게 language embedding 역시 convolution layer에 통과시킨뒤 text representation에 더하는 식으로 language 정보를 모듈에 줍니다.


Text encoder에서는 Nekvinda and Dusek<a href="#r4"><sup>[4]</sup></a>가 제안한 방법인 parameter generation을 사용해 language정보를 제공합니다.
이 방법은 하나의 text encoder 모듈에서 다양한 언어에 맞는 text representation을 생성하도록 만들어줍니다.
Parameter generator는 language embedding을 받아 언어 정보가 들어가있는 parameter를 생성한 다음, parameter들을 text encoder의 각 부분에 더합니다.
...
이러한 편향문제를 막기 위해 기존 다국어 TTS 모델들<a href="#r4"><sup>[4]</sup></a><a href="#r5"><sup>[5]</sup></a>은 domain adversarial traing (DAT)<a href="#r6"><sup>[6]</sup></a>을 도입했습니다.
SANE-TTS 역시 DAT를 적용하여 위의 문제를 해결했습니다.
SANE-TTS에 DAT를 적용한 자세한 방법은 아래에 나와있습니다.
...
다국어 TTS 모델 훈련데이터에서 보통 한 화자는 단일언어에 대해서만 음성데이터가 있기 때문에 화자 정보가 언어에 편향될 수 있습니다.
...
이 loss를 통해 hidden speaker representation인 $\mathrm{conv}(S_{k})$의 평균이 언어에 관계없이 영벡터로 가게 되고 화자정보는 언어와 분리되게 됩니다.
...
영어 화자가 한국어 음성을 말할 때 기본적인 톤, 세기등은 어느정도 예측이 가능하지만 발화 duration은 예측하기 힘듭니다.
그 이유는 톤, 세기 같은 특성들은 화자특성에 가까워 언어와 큰 관련이 없지만 발화 duration은 언어와 크게 맞닿아 있기 때문입니다.
따라서 다국어 TTS 모델에서 영어 화자의 특성을 그대로 한국어 음소의 duration을 예측하는데 사용한다면 부자연스럽게 들릴뿐 화자특성을 반영했다고 보기 어렵습니다.
...
이를 이용해 cross-lingual speech synthesis시 duration predictor에 speaker embedding 대신 영벡터를 집어넣습니다.
...
특히 cross-lingual speech synthesis시 SANE-TTS는 화자에 관계없이 일정한 duration을 예측하기 때문에 안정적인 음성을 합성할 수 있습니다.
...
| :----- | :--------------: | :-------------: | :-------------: | :-------------: | :-------------: |
| LJ | <audio controls width="100" src={audiogt_1}></audio> | <audio controls> <source src={audioen_1}/> </audio> | <audio controls> <source src={audioko_1}/> </audio> | <audio controls> <source src={audioja_1}/> </audio> | <audio controls> <source src={audiozh_1}/> </audio> |
| LibriTTS<br/>4356 | <audio controls src={audiogt_2}></audio> | <audio controls> <source src={audioen_2}/> </audio> | <audio controls> <source src={audioko_2}/> </audio> | <audio controls> <source src={audioja_2}/> </audio> | <audio controls> <source src={audiozh_2}/> </audio> |
| KSS | <audio controls src={audiogt_3}></audio> | <audio controls> <source src={audioen_3}/> </audio> | <audio controls> <source src={audioko_3}/> </audio> | <audio controls> <source src={audioja_3}/> </audio> | <audio controls> <source src={audiozh_3}/> </audio> |
| jvs001 | <audio controls src={audiogt_4}></audio> | <audio controls> <source src={audioen_4}/> </audio> | <audio controls> <source src={audioko_4}/> </audio> | <audio controls> <source src={audioja_4}/> </audio> | <audio controls> <source src={audiozh_4}/> </audio> |
| SSB0073 | <audio controls src={audiogt_5}></audio> | <audio controls> <source src={audioen_5}/> </audio> | <audio controls> <source src={audioko_5}/> </audio> | <audio controls> <source src={audioja_5}/> </audio> | <audio controls> <source src={audiozh_5}/> </audio> |
